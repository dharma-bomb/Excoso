...
(function () {
  ...
  function renderEdit(products, categories) {
    ...
    el.innerHTML = featured.slice(0, 6).map(function (p, i) {
      ...
      if (img) {
        return (
          '<a href="product.html?id=' + encodeURIComponent(p.id) + '" class="p-card p-card--photo">' +
      ...
      return (
        '<a href="product.html?id=' + encodeURIComponent(p.id) + '" class="p-card p-card--' + v + '">' +
  ...
  // ---------- PROMO BANNERS (homepage only, Firestore `banners` collection) ----------
  function renderBanners(banners) {
    var el = document.getElementById('promoBanners');
    if (!el) return;
    ...
    el.style.display = '';
    el.innerHTML = '<div class="wrap promo-banners__track">' + active.map(function (b) {
      return (
        '<a href="#quote" class="promo-banner">' +
          (b.image ? '<img src="' + esc(b.image) + '" alt="' + esc(b.title) + '">' : '') +
          '<div class="promo-banner__text">' +
            '<span class="promo-banner__title">' + esc(b.title) + '</span>' +
            (b.subtitle ? '<span class="promo-banner__subtitle">' + esc(b.subtitle) + '</span>' : '') +
          '</div>' +
        '</a>'
      );
    }).join('') + '</div>';
  ...
  // ---------- FOOTER SETTINGS (company info, Firestore `settings/main` doc) ----------
  function renderFooterSettings(settings) {
    if (!settings) return; // no doc yet (or static-file mode) — leave the hardcoded fallback markup as-is
    var listEl = document.getElementById('footerContactList');
    if (listEl) {
      var rows = [];
      if (settings.email) rows.push('<li><a href="mailto:' + esc(settings.email) + '">' + esc(settings.email) + '</a></li>');
      if (settings.phone) rows.push('<li><a href="tel:' + esc(settings.phone.replace(/[^+\d]/g, '')) + '">' + esc(settings.phone) + '</a></li>');
      if (settings.whatsapp) rows.push('<li><a href="https://wa.me/' + esc(settings.whatsapp.replace(/[^\d]/g, '')) + '" target="_blank" rel="noopener">WhatsApp</a></li>');
      if (settings.address) rows.push('<li>' + esc(settings.address) + '</li>');
      if (settings.instagram) rows.push('<li><a href="' + esc(settings.instagram) + '" target="_blank" rel="noopener">Instagram</a></li>');
      if (settings.linkedin) rows.push('<li><a href="' + esc(settings.linkedin) + '" target="_blank" rel="noopener">LinkedIn</a></li>');
      if (rows.length) listEl.innerHTML = rows.join('');
    }
    var companyLineEl = document.getElementById('footerCompanyLine');
    if (companyLineEl && settings.companyName) companyLineEl.textContent = 'A brand by ' + settings.companyName;
  }
  ...
  function renderAll(data) {
    ...
    renderFooterSettings(data.settings);
  ...
  function loadFromFirestore() {
    return db.collection('categories').get().then(function (catSnap) {
      return db.collection('products').get().then(function (prodSnap) {
        return Promise.all([
          db.collection('banners').get().catch(function () { return { docs: [] }; }),
          db.collection('settings').doc('main').get().catch(function () { return { exists: false }; })
        ]).then(function (extra) {
          var bannerSnap = extra[0], settingsDoc = extra[1];
          var products = prodSnap.docs.map(function (d) { return Object.assign({ id: d.id }, d.data()); })
            .filter(function (p) { return p.active !== false; }); // hide inactive products from the public site
          return {
            categories: catSnap.docs.map(function (d) { return Object.assign({ id: d.id }, d.data()); }),
            products: products,
            banners: bannerSnap.docs.map(function (d) { return Object.assign({ id: d.id }, d.data()); }),
